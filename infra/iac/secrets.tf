# Este arquivo define Secrets no Kubernetes usando Terraform, que armazenam informações sensíveis, como credenciais, de forma segura.
# Secret que armazena credenciais para acessar o GitHub Container Registry (GHCR) a partir do Jenkins

# This file defines Secrets in Kubernetes using Terraform, which store sensitive information, such as credentials, securely.
...
resource "kubernetes_secret" "ghcr" {
  ...
  # Valores sensíveis armazenados no Secret (codificados em base64 pelo Terraform):
  # Sensitive values stored in the Secret (base64 encoded by Terraform):
  data = {
